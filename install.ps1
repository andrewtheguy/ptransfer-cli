...
#
# Windows support is beta: the build is produced and installed the same way the
# others are, but it gets far less use than Linux and macOS. Run it under
# PowerShell 7, which is what this installer checks for below.
#
...
Supported platforms: Windows (amd64), beta - PowerShell 7 recommended
...
#
# Windows PowerShell 5.1 ships with the operating system and runs both this
# script and the CLI, so this is a warning rather than a refusal. PowerShell 7
# is what the CLI is exercised under: it is the default shell in Windows
# Terminal, where the TUI gets a real virtual-terminal console and Ctrl+Shift+C
# and Ctrl+Shift+V copy and paste a transfer code whole; the right-click of
# QuickEdit mode truncates one, and the transfer then fails on a code that
...
function Test-PowerShellVersion {
    ...
    Print-Warn "Running under PowerShell $($PSVersionTable.PSVersion); PowerShell 7 is recommended."
    Print-Warn "Install it with: winget install --id Microsoft.PowerShell --source winget"
    Print-Warn "Then reopen your terminal with 'pwsh' and run pTransfer from there."
}
...
function Start-Installation {
    ...
    if ($DownloadOnly) {
        ...
        Print-Info "Windows Terminal with PowerShell 7 is the recommended place to run it."
        Print-Info "Copy and paste transfer codes with Ctrl+Shift+C and Ctrl+Shift+V:"
        Print-Info "a right-click copy or paste truncates a code and the transfer fails."
    }
}
...
function Main {
    ...
    Print-Warn "Windows support is beta; please report anything that misbehaves."
